...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased">
        @php
            $navLinks = [
                ...
                ['route' => 'schedule.index', 'label' => 'Reservas', 'icon' => 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z',
                    ...
                    'children' => [
                        ...
                        ['route' => 'payment.index', 'label' => 'Pagamentos'],
                    ],
                ],
            ...
            // Cachê de funcionários. Como no Banco de Horas, quem entra é quem
            // tem vínculo de setor — não há permissão do Spatie para isso.
            $cacheAccess = \App\Support\EmployeeScope::for(auth()->user());

            if ($cacheAccess['type'] !== 'none' || (auth()->user()?->can('manage-employee-cache-payments'))) {
                $cacheChildren = [['route' => 'employee-caches.index', 'label' => 'Cachês']];

                if (\App\Support\EmployeeScope::isCoordinator($cacheAccess)) {
                    $cacheChildren[] = ['route' => 'employee-caches.create', 'label' => 'Solicitar'];
                    $cacheChildren[] = ['route' => 'employee-caches.batches', 'label' => 'Solicitações'];
                    $cacheChildren[] = ['route' => 'employee-caches.recheck.queue', 'label' => 'Reconferência'];
                }

                if (auth()->user()?->isManagementCoordinator()) {
                    $cacheChildren[] = ['route' => 'employee-caches.queue', 'label' => 'Aprovação'];
                }

                if (auth()->user()?->can('manage-employee-cache-payments')) {
                    $cacheChildren[] = ['route' => 'employee-caches.finance', 'label' => 'Financeiro'];
                }

                $navLinks[] = [
                    'route' => $cacheChildren[0]['route'],
                    'label' => 'Cachê',
