...
<div class="max-w-5xl mx-auto">
        ...
        <div class="px-8 pb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-2">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 font-medium leading-relaxed">{{ $companyDetails->description }}</p>
                </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
        ...
        <div id="content-workers" class="tab-content p-6"
            ...
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-xl font-extrabold text-gray-800 dark:text-white">
                    Equipe Registrada
                    <span class="ml-1 text-sm font-normal text-gray-400">({{ $companyDetails->workers->count() }})</span>
                </h3>
            ...
            <div class="relative mb-5">
                ...
                <input x-model="search" type="text" placeholder="Pesquisar por nome ou cargo..."
                    class="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 transition">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @if($companyDetails->workers->isEmpty())
                    <p class="text-gray-500 dark:text-gray-400 italic col-span-2">Nenhum funcionário registado para esta empresa.</p>
                @endif
                ...
                <div class="flex items-center p-4 border border-gray-100 dark:border-gray-700 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition shadow-sm"
                    ...
                    @else
                        <div class="h-12 w-12 rounded-full bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center text-indigo-700 dark:text-indigo-400 font-bold mr-4 shrink-0">
            ...
            <p x-show="filteredCount === 0 && search !== ''"
               class="text-center text-gray-400 dark:text-gray-500 italic py-6">
        ...
        <div id="content-rules" class="tab-content p-6 hidden"
            ...
            <div class="flex gap-1 p-1 bg-gray-100 dark:bg-gray-700/50 rounded-xl mb-5">
                <button @click="switchRuleTab('vigentes')"
                    ...
                    :class="ruleTab === 'vigentes' ? 'bg-white dark:bg-gray-600 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    Vigentes
                    <span class="ml-0.5 px-1.5 py-0.5 rounded-full text-[10px]"
                          :class="ruleTab === 'vigentes' ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400' : 'bg-gray-200 dark:bg-gray-600 text-gray-500 dark:text-gray-400'">
                        {{ $vigentes->count() }}
                    </span>
                </button>
                <button @click="switchRuleTab('expiradas')"
                    class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold transition"
                    :class="ruleTab === 'expiradas' ? 'bg-white dark:bg-gray-600 text-red-600 dark:text-red-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    Expiradas
                    <span class="ml-0.5 px-1.5 py-0.5 rounded-full text-[10px]"
                          :class="ruleTab === 'expiradas' ? 'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400' : 'bg-gray-200 dark:bg-gray-600 text-gray-500 dark:text-gray-400'">
                        {{ $expiradas->count() }}
                    </span>
                </button>
                <button @click="switchRuleTab('todas')"
                    class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold transition"
                    :class="ruleTab === 'todas' ? 'bg-white dark:bg-gray-600 text-gray-700 dark:text-gray-300 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"/></svg>
                    Todas
                    <span class="ml-0.5 px-1.5 py-0.5 rounded-full text-[10px]"
                          :class="ruleTab === 'todas' ? 'bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300' : 'bg-gray-200 dark:bg-gray-600 text-gray-500 dark:text-gray-400'">
                        {{ $allRules->count() }}
                    </span>
                </button>
            </div>
            ...
            <div class="flex items-center justify-between mb-4 min-h-[36px]">
                <label class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 cursor-pointer select-none">
                    ...
                    <span x-show="selected.length > 0" class="text-xs text-indigo-600 dark:text-indigo-400 font-semibold">
                ...
                <form x-show="selected.length > 0"
                    ...
                    <button type="button"
                            ...
                            class="flex items-center gap-1.5 px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded-lg text-xs font-bold transition shadow-sm">
            ...
            @forelse($allRules as $rule)
                ...
                <div class="mb-4"
                    ...
                    <div class="bg-white dark:bg-gray-700 border rounded-2xl shadow-sm overflow-hidden border-gray-100 dark:border-gray-600"
                         :class="selected.includes({{ $rule->id }}) ? 'ring-2 ring-indigo-400 dark:ring-indigo-500' : ''">
                        <div class="flex flex-col md:flex-row">
                            ...
                            <div class="flex items-center gap-0 md:flex-col">
                                ...
                                <div class="flex-1 md:flex-none w-full md:w-2 h-2 md:h-full {{ $rule['type'] === 'include' ? 'bg-indigo-600' : 'bg-red-600' }} md:rounded-none"></div>
                            </div>
                                ...
                                <div class="flex flex-wrap items-start justify-between gap-4">
                                    <div class="flex-grow">
                                        <div class="flex flex-wrap items-center gap-2 mb-1">
                                            ...
                                            @if(!$isVigente)
                                                <span class="px-2 py-0.5 rounded text-[10px] font-black uppercase bg-gray-100 dark:bg-gray-600 text-gray-500 dark:text-gray-400">
                                                    Expirada
                                                </span>
                                            @endif
                                            ...
                                            @endif
                                            <h4 class="font-extrabold text-gray-900 dark:text-white text-base">{{ $rule['description'] }}</h4>
                                        </div>
                                        ...
                                        <div class="flex items-center text-xs text-gray-500 dark:text-gray-400 mb-3">
                                            ...
                                            @if($rule['end_date'])
                                                ...
                                                <span class="font-bold {{ !$isVigente ? 'text-red-500 dark:text-red-400' : 'text-gray-700 dark:text-gray-300' }}">
                                        ...
                                        <div class="flex gap-1.5 mb-3">
                                            @foreach(['seg', 'ter', 'qua', 'qui', 'sex', 'sab', 'dom'] as $d)
                                                <span class="w-8 h-8 flex items-center justify-center rounded-lg text-[10px] font-black uppercase transition-colors {{ in_array($d, $rule['weekdays']->pluck('short_name_pt')->toArray()) ? 'bg-indigo-600 text-white shadow-sm' : 'bg-gray-100 dark:bg-gray-600 text-gray-300 dark:text-gray-500' }}">
                                                    {{ $d }}
                                                </span>
                                            @endforeach
                                        </div>
                                        ...
                                        @if($rule->start_time && $rule->end_time)
                                            <div class="flex items-center text-xs text-gray-500 dark:text-gray-400 mb-2">
                                                ...
                                                Horário: <span class="mx-1 font-bold text-gray-700 dark:text-gray-300">{{ date('H:i', strtotime($rule['start_time'])) }}</span> / <span class="font-bold text-gray-700 dark:text-gray-300">{{ date('H:i', strtotime($rule['end_time'])) }}</span>
                                            </div>
                                        @endif
            ...
            @empty
                <p class="text-gray-500 dark:text-gray-400 italic text-center py-8">Nenhuma regra de acesso cadastrada.</p>
            @endforelse

            <!-- Sem resultados na sub-aba -->
            <p x-show="visibleIds().length === 0" class="text-center text-gray-400 dark:text-gray-500 italic py-6">
