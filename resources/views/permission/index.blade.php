...
<div class="py-12 bg-gray-50 dark:bg-gray-900 min-h-screen">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <a href="{{ route('roles-permission.create') }}" class="inline-flex items-center px-6 py-3 bg-[#A00001] text-white rounded-xl font-bold shadow-lg hover:bg-[#800000] transition duration-150 transform hover:scale-[1.02]">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 flex flex-col role-card overflow-hidden transition-all duration-300 hover:shadow-indigo-100 dark:hover:shadow-indigo-900/20">
                ...
                <div class="p-6 border-b border-gray-50 dark:border-gray-700 flex items-center justify-between bg-gray-50/50 dark:bg-gray-700/50">
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('roles-permission.show', $role['id']) }}" class="p-2 text-indigo-600 dark:text-indigo-400 hover:bg-white dark:hover:bg-gray-600 rounded-lg transition border border-transparent hover:border-indigo-100 dark:hover:border-indigo-900 hover:shadow-sm" title="Editar Permissões">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                        </a>
                        @if($role['name'] !== 'admin')
                        <form method="POST" action="{{ route('roles-permission.destroy', $role['id']) }}"
                              onsubmit="return confirm('Excluir o grupo \'{{ $role['name'] }}\' permanentemente? Esta ação não pode ser desfeita.')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="p-2 text-red-700 dark:text-red-400 hover:bg-white dark:hover:bg-gray-600 rounded-lg transition border border-transparent hover:border-red-100 dark:hover:border-red-900 hover:shadow-sm" title="Excluir Grupo">
                ...
                <div class="p-6 flex-grow">
                    ...
                    <div class="flex flex-wrap gap-2 permission-list">
                        ...
                        @empty
                            <span class="text-sm text-gray-400 dark:text-gray-500 italic">Nenhuma permissão atribuída.</span>
                        @endforelse
                    </div>
                </div>
...
<script>
    function filterRoles() {
        ...
        cards.forEach(card => {
            ...
            card.style.display = (roleName.includes(filter) || permissions.includes(filter)) ? "" : "none";
        });
    }
</script>
