...
- `php artisan db:seed --class=PlacarVoleiClubeFlamengoSeeder` — uma **partida de vôlei inteira, já jogada**: Clube dos Funcionários 3 x 1 Flamengo (25x20, 22x25, 25x23, 25x19), com escalação dos dois lados, ~200 eventos, tempos técnicos, substituições e um cartão. O **Flamengo é criado por completo** (equipe, time de vôlei adulto e 12 jogadores com número, posição e data de nascimento); o **Clube dos Funcionários só é localizado** — o seeder nunca cria essa equipe, para não duplicar cadastro real, e para com uma mensagem se ela não existir. É a massa mais direta para conferir súmula, súmula por time, sets e minutagem. Idempotente.
